...
# df2 = [i for i in df['text'] if len(i.split())>100]
# seq_len = [len(i.split()) for i in df['text']]
...
# Split text in half if it has more than 512 words
# index = 0
# while index < len(df.index):
#
#     # if text has more than 300 words
#     if len(str(df.loc[index, 'text']).split()) > 100:
#         word_index = 50
#         i = str(df.loc[index, 'text']).split()[word_index]
#
#         # Find full stop to separate the text
#         while (not "." in i) and (not ";" in i):
#             word_index += 1
#             i = str(df.loc[index, 'text']).split()[word_index]
#
#         # If there isn't space between two sentences, then separate the sentences correctly
#         split_after = ""
#         split_before = ""
#         if len(i.split(".")) > 1:
#             split_after = i.split(".")[-1]
#             split_before = ''.join(i.split(".")[:-1]) + "."
#         part1 = ' '.join(str(df.loc[index, 'text']).split()[:word_index]) + " " + str(split_before)
#         part2 = str(split_after) + " " + ' '.join(str(df.loc[index, 'text']).split()[word_index + 1:])
#
#         # Assign the first part in the existing row and the second one to a new one
#         df.loc[index, 'text'] = part1
#         df3 = pd.DataFrame({"name": [df.loc[index, 'name']],
#                             "text": [part2],
#                             "political_party": [df.loc[index, "political_party"]]})
